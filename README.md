...
- **恢复**：每轮提交前先写入“未确认”标记。进程退出或协议错误等结果不明确时会话暂停；打开会话时自动回放原生历史核对，从未确认轮起每轮都有回复或工具活动才视为已执行并解除暂停。无法确认时，Harness 胶囊旁出现 **恢复会话**，可选 **核对原生记录** 或 **解除暂停，不重发**；插件绝不自动重发原请求。
...
- ACP 不传输账户额度，因此保留两处原生补充：`src/codex-rpc.ts`（Codex app-server）与 `src/claude-sdk.ts`（Claude Agent SDK 与可执行文件发现）。
...
- ACP 回放不含每轮的结束状态：有输出的轮次一律视为成功，没有输出的轮次在恢复核对时保持未知，需要用户明确解除暂停。回放出的工具项不含输出。
- DSH 消息来源的 Provider 记为 `unreported`（ACP 不报告）。
- ACP 已公布但未接入界面的能力：Codex 快速模式与计划协作模式、目标（goal）扩展、原生子代理会话、后台终端任务、会话列表/删除。
- 普通文件附件需要本地文件存储后端。
