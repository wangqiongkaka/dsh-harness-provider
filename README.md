...
- 显示上下文用量、账户额度（会话切换模型厂商后立即刷新）、输出速度、token 和缓存命中。
...
在输入框的“指令”菜单中选择 **委派**，或输入 `/delegate`，再在原输入框中填写任务。任务可以同时携带图片和文件；发送前可选择 DSH 原生、Codex 或 Claude Code，并决定是否在完成后回传到当前会话。
...
- 主 Agent 根据任务是否可并行，自行选择 1–4 个 DSH 原生、Codex 或 Claude Code 会话并划分任务。
