...
const codex=await startResponsesFixture([{kind:'complete',text:'Codex native first'},{kind:'complete',text:'Codex native second'},{kind:'complete',text:'Codex native resumed'},{kind:'complete',text:'Codex after rollback'},{kind:'complete',text:'Codex after edit'}]);
...
const env={PATH:process.env.PATH,HOME:root,DSH_HOME:dshHome,DSH_TELEMETRY_DISABLED:'1',CODEX_HOME:codexHome,
 ...
 CLAUDE_COMMAND_PATH:process.env.CLAUDE_COMMAND ?? '/opt/homebrew/bin/claude',CLAUDE_CONFIG_DIR:claudeHome,
...
try{
 ...
 // Editing a finished prompt reruns from it in place; a rolled-back turn has no native boundary and offers no edit.
 const userRow=text=>page.locator('[data-chat-flow-kind="user"]').filter({hasText:text}).first();
 await userRow('Continue after restart').hover();
 await expect(userRow('Continue after restart').getByRole('button',{name:'Edit',exact:true})).toHaveCount(0);
 await userRow('Continue with second marker').hover();
 await page.screenshot({path:resolve('.cache/edit-button-web.png')});
 await userRow('Continue with second marker').getByRole('button',{name:'Edit',exact:true}).click();
 const editBox=page.getByRole('textbox',{name:'Edit',exact:true});
 await expect(editBox).toHaveValue('Continue with second marker');
 await editBox.fill('Second marker edited');
 await page.screenshot({path:resolve('.cache/edit-box-web.png')});
