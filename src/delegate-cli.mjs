...
try {
  ...
  if (!['create', 'read', 'discuss'].includes(method) || !value || extra.length) throw new Error('Usage: delegate-cli.mjs create <JSON> | read <sessionId> | discuss <JSON>');
  ...
  const body = method !== 'read' || value.startsWith('{') ? JSON.parse(value) : { sessionId: value };
  const response = await fetch(new URL(method, url), {
    method: 'POST', headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' },
    body: JSON.stringify(body), signal: AbortSignal.timeout(method === 'discuss' ? 1_800_000 : 120_000),
  });
