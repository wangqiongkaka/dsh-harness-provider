...
- 验证：`npm run check` 的 43 项测试全部通过；临时 DSH Web Profile 中，委派与讨论位于同一 Commands 分组，讨论输入条及原委派图片/回传流程通过。浏览器模型请求均由本地 fixture 响应。
...
## 外部 Harness 额度改为 stale-while-revalidate（2026-09-19）

### 问题

- Codex / Claude Code 的额度探测要为每次读取冷启动一个一次性 CLI 进程（实测 Codex ~1.4s、Claude ~2.5s）；切换会话后最长要等这几秒才显示，且停留在外部 Harness 会话时「60 秒轮询 + 60 秒缓存」等于每分钟起一个进程。

### 决定

- 新增 `HarnessQuotaCache`（每个 harness 一份，`dsh.ts` 导出以便测试）：读取直接返回缓存；只有无缓存（首次）或已过最早窗口的 `resetsAt` 时才探测并等待结果，并发读取共用同一次探测；每轮回合结束都在后台探测（包括该 Harness 从未读取过额度时），期间读取返回旧值；其他时候（切换会话、客户端轮询、长时间闲置）不探测。探测失败时保留仍有效的旧值，否则记为无数据，直到下一回合结束再重试。
- 刷新时机放在回合结束而不是提交时刻：回合结束才是用量真正确定（含本轮消耗）的时刻，探测在后台与一切并行。DSH 原生会话的回合不触碰该缓存；原生 provider（DeepSeek/GLM）仍是廉价 HTTP 探测，维持原 60 秒缓存不变。
- 面板脚注按来源区分：外部 Harness 显示「对话一轮后同步」，原生 provider 保留「每 60 秒同步」。

### 验证

- `tests/quota.test.mjs` 新增两组时钟注入的单测：首次并发读取共用一次探测、重置前长时间闲置不探测、回合结束后台刷新且期间读取返回旧值、过重置时间读取探测一次且不重复、首次失败记为无数据直到下一回合、后台失败保留有效旧值且无重试风暴。新测试在旧的“闲置 10 分钟补探”实现上失败。
- `tests/dsh-service.test.mjs` 新增服务级测试：绑定 Codex 后首次读取内联探测、重复读取不探测；`turn/end` 事件后无需任何读取即触发后台探测并落地新值；切回 DSH 原生后的回合不再探测。
- `npm run check` 通过（52 项测试，含类型检查与构建）。

## 空闲 Harness 会话回收（2026-09-19）

### 问题

- 外部 Harness 进程持有原生对话，只要 DSH 会话不删除就常驻；长期挂着的 Codex / Claude Code 会话一直占着一个 CLI 进程及其内存。

### 决定

- `DshRunner` 增加空闲回收：默认 10 分钟无活动即关闭该会话进程并移出 `live` 表，下一回合按绑定的 `nativeRef` 以 `kind: 'resume'` 重新拉起（恢复通道本来就有），代价是回收后首条消息多 2~4 秒冷启动。
- 活动时间在创建会话、回合开始与回合结束（`finally`）时刷新；`turnId` 存在（回合进行中）、agent 正在运行、收件箱有待处理消息、或上次结果未确认（`binding.pending`，用户即将核对原生记录）都不回收。读绑定期间回合重新开始的竞态用 `live` 身份比较挡掉。
- 扫描周期取 `min(60s, 空闲窗口/4)`，既省电也让测试能用短窗口验证；回收器与其他生命周期一样挂在 `ctx.effect` 上随插件作用域销毁。
- 核实过「复用常驻进程做额度探测」不可行：`codex-acp` 只把账户限额留在自己的会话状态里（供 `/status` 文本用），不经 ACP 暴露给客户端；`claude-agent-acp` 完全没有限额字段；ACP 扩展方法只有 `_session/goal` 与 `_session/steering`。探测仍需自起进程，维持既有的 stale-while-revalidate 频率。

### 验证

- `tests/dsh-runner.test.mjs` 新增：120ms 空闲窗口下，会话在没有任何提示的情况下被关闭（`live` 表清空），下一回合以 `resume` 重新打开且原生身份不变；回合进行中即使远超空闲窗口也不回收。把扫描周期临时改成一小时该测试即失败，确认回归辨识力。
- `npm run check` 通过（53 项测试，含类型检查与构建）。

### 修订：1 分钟回收、查看保活、后台任务保活

- 空闲窗口改为 1 分钟，扫描周期随之为 15 秒。空闲计时取「上一回合结束」与「最后一次查看上报」中较晚者。
- 查看保活：前端 `HarnessSelect` 在外部 Harness 会话且页面可见时，挂载即上报、之后每 15 秒上报一次（`harness/viewing`），页面恢复可见时立即补报；页面隐藏或关闭即停止上报。心跳间隔远小于空闲窗口，因此不需要单独的「查看有效期」，离开后 45~75 秒回收。
- 后台任务保活：两个适配器都通过 AIR `asyncTasks` 能力发布后台任务生命周期（`claude-agent-acp` 0.78.0 `dist/async-tasks.js`，`codex-acp` 1.12.0 `CodexBackgroundTerminalTasks`），客户端声明该能力后才发送。ACP SDK 1.4.0 的 schema 不认识 `async_task_*`，与子代理生命周期一样在线路上改名放行。`AcpSession` 记录 running / paused 的任务，`hasBackgroundTasks()` 为真时不回收，并把空闲计时重置到当前，任务全部结束后再等一个空闲窗口。后台子代理无需额外处理：`claude-agent-acp` 在其结束前保持回合未结束（`isHeldOpen`），`turnId` 一直存在。
- 子代理卡片：回收前把 `subagents()` 的最后结果留在 `DshRunner.retainedSubagents`（内存），无存活进程时 `harness/subagents` 读它；会话恢复或 agent 销毁时清除。未写入 binding，因为 binding 约定不保存对话副本；DSH 重启后该快照丢失，卡片回到空列表。
- 不做切换会话时预先 resume：对话、上下文用量、模型设置均不依赖存活进程，仅第一条消息多一次冷启动。
